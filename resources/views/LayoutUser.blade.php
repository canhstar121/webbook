...
<body class="home-2 home-3">
                        ...
                        <div class="col-lg-6 col-md-6 col-12">
                            <div class="account-area text-end">
                                <ul>
                                    ...
                                    @else
                                        <li><a href="{{ route('authuser.index') }}">Đăng Nhập</a></li>
                                    @endif
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-3 col-md-6 col-12">
                            <div class="logo-area logo-xs-mrg-bottom">
                                <a href="{{ route('home.index') }}"><img src="{{ asset('User/img/logo/6.png') }}"
                                        alt="logo" /></a>
                            </div>
                        </div>
                            ...
                            <div class="main-menu hidden-sm hidden-xs">
                                <nav>
                                    <ul>
                                        ...
                                        <li><a href="#">Chi Tiết<i class="fa fa-angle-down"></i></a>
                                            <div class="sub-menu sub-menu-2">
                                                <ul>
                                                    <li><a href="{{ route('home.create') }}">Tất Cả Sách</a></li>
                                                    <li><a href="{{ route('cart.create') }}">thanh toán</a></li>
                                                    <li><a href="{{ route('link.index') }}">yêu thích</a></li>
                                                    <li><a href="{{ route('link.create') }}">so sánh</a></li>
                                        ...
                                        <li><a href="{{ route('shared.create') }}">Giới Thiệu</a></li>
                                        <li><a href="{{ route('shared.index') }}">Liên Hệ</a></li>
                    ...
                    <div class="row">
                        <div class="col-lg-3 col-md-4">
                            <div class="category-area category-mb">
                                <h3><a href="#" id="showcat">Thể loại sách <i class="fa fa-bars"></i></a>
                                </h3>
                                <div class="category-menu category-menu-1"
                                    style="{{ request()->url() != route('home.index') ? 'display: none;' : 'display: block;' }}"
                                    id="hidecat">
                        ...
                        <div class="col-lg-9 col-md-8">
                            <div class="header-bottom-search">
                                <form action="{{ route('search.index') }}" method="GET">
                                    <input type="text" name="txtSearch" placeholder="Tìm mọi thứ ở đây..." />
                                    {{--  <a ><i class="fa fa-search"></i></a>  --}}
                                    <button type="submit"><i style="display: block;"
                                            class="fa fa-search"></i></button>
                                    <div class="pos-search">
                                        <select class="bootstrap-select" name="txtCate">
                                            <option value="0">Tất cả</option>
                                            @foreach ($categoriesSearch as $item)
                                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                                @if ($item->ParentCate->count() > 0)
                                                    @include('User.inc.option_category', [
                                                        'item' => $item,
                                                        'text' => '-',
                                                    ])
                                                @endif
                        ...
                        <div class="col-lg-4 col-12">
                            <div class="single-footer mrg-sm">
                                ...
                                <div class="footer-contact">
                                    ...
                                    <p><span>Gọi ngay cho chúng tôi:</span> 0123456789</p>
    ...
    <script>
        ...
        function doDuLieuSanPham(id) {
            danhSachSanPham = {!! $sanpham !!};
            danhSachSanPham.forEach(function(row, key) {
                if (row['id'] == id) {
                    $('#productModal').modal('show');
                    $('#nameModal').text(row['name']);
                    $('#priceModal').text(row['price']);
                    {{--  $('#descModal').text(row['description']);  --}}

                    $('#imageModal').attr('src', 'uploads/sanpham/' + row['imageDefault']);
                    $('.imageSubModal').attr('src', 'uploads/sanpham/' + row['imageDefault']);

                    if (row['image'] != null) {
                        $('.arrImg').show();
                        $('.arrImgSub').show();
                        row['image'].split("|").forEach(function(value, item) {
                            $('.imageArrModal' + item).attr('src', 'uploads/sanpham/' + value);
                            $('.imageArrSubModal' + item).attr('src', 'uploads/sanpham/' + value);
                        });
                    } else {
                        $('.arrImg').hide();
                        $('.arrImgSub').hide();
